...
## - Player walks between different elevation levels
## - Buildings placed on elevated ground are at correct height
## - Drill + conveyor + sink production chain works on elevated terrain
## - Player walks back down from elevated terrain
## - Height transitions render correctly (visual screenshots)
...
func run_scenario() -> void:
	...
	await monitor.screenshot("03_on_plateau")

	# ── Test 4: Place buildings on elevated terrain ─────────────────────
	# Place drill on the deposit at the elevated plateau
	...
	# Place conveyor line and sink via place_at (no walking through buildings)
	...
	var sink_building = GameManager.get_building_at(Vector2i(22, 12))
	if sink_building:
		var sink_y: float = sink_building.position.y
		assert_scenario(sink_y > 0.8, "Sink at correct height (y=%.3f)" % sink_y)

	await monitor.screenshot("04_buildings_on_plateau")

	# ── Test 5: Production chain works on elevated terrain ──────────────
	await bot.wait(12.0)
	monitor.sample()
	...
	await monitor.screenshot("05_production_running")

	# ── Test 6: Walk back down to flat ground ───────────────────────────
	arrived = await bot.walk_to(Vector2i(8, 12))
	assert_scenario(arrived, "Walked back to flat ground")
	await bot.wait(0.5)
	monitor.sample()
	var y_back_flat: float = player.position.y
	assert_scenario(y_back_flat < 0.3, "Player back at ground level (y=%.3f)" % y_back_flat)
	await monitor.screenshot("06_back_on_flat")

	# ── Test 7: Teleport to high plateau (1.5) ─────────────────────────
	# Teleport past buildings to reach the high plateau
	...
	await monitor.screenshot("07_high_plateau")

	# ── Test 8: Verify height differences are significant ───────────────
	...
	assert_gt_scenario(y_on_high - y_on_plateau, 0.3,
		"High plateau higher than 1.0 plateau (diff=%.3f)" % (y_on_high - y_on_plateau))
