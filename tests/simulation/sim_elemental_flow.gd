...
func run_simulation() -> void:
	...
	#
	# Layout (smelter anchor at (20,10), default rotation = right):
	#   Smelter shape: (20,9),(21,9),(20,10),(20,11),(21,11)
	#   Smelter input cells: In_0 at (20,10) from LEFT, In_4 at (20,9) from LEFT/UP/DOWN
	#   Smelter output gap: (21,10) — conveyor placed here
	#
	#   Drill A on pyromite at (10,10), outputs right
	#   Conveyors: (11,10)...(19,10) going right → feeds into smelter In_0
	#
	#   Drill B on crystalline at (10,9), outputs right
	#   Conveyors: (11,9)...(19,9) going right → feeds into smelter In_4
	#
	#   Output: conveyor at (21,10), (22,10) → sink at (23,10)
	...
	var drill_b = sim_place_building(&"drill", Vector2i(10, 9), 0)
	sim_assert(drill_b != null, "Drill B placed on crystalline deposit")

	# --- Conveyor chains to smelter inputs ---
	# Bottom chain: feeds pyromite into smelter input at (20,10)
	for x in range(11, 20):
		sim_place_building(&"conveyor", Vector2i(x, 10), 0)

	# Top chain: feeds crystalline into smelter input at (20,9)
	for x in range(11, 20):
	...
	# --- Output chain ---
	sim_place_building(&"conveyor", Vector2i(21, 10), 0)  # output gap
	sim_place_building(&"conveyor", Vector2i(22, 10), 0)
	sim_place_building(&"sink", Vector2i(23, 10), 0)

	# --- Run and verify ---
	# Drills produce every 2s. Smelter needs 1 pyromite + 1 crystalline, crafts in 3s.
	# After ~15s we should see steam_burst items reaching the sink.
	await sim_advance_seconds(15)

	var sink_building = GameManager.get_building_at(Vector2i(23, 10))
	...
	sim_add_deposit(Vector2i(40, 9), &"crystalline")

	sim_place_building(&"drill", Vector2i(40, 10), 0)
	sim_place_building(&"drill", Vector2i(40, 9), 0)
	...
	sim_place_building(&"conveyor", Vector2i(51, 10), 0)
	sim_place_building(&"conveyor", Vector2i(52, 10), 0)
	sim_place_building(&"sink", Vector2i(53, 10), 0)

	await sim_advance_seconds(15)

	var sink2 = GameManager.get_building_at(Vector2i(53, 10))
