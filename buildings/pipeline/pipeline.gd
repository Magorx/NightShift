...
var max_tunnel_gap: int = 9

var direction: int = 0 # 0=right, 1=down, 2=left, 3=up
var is_input: bool = true # true = input end, false = output end

## Reference to the paired pipeline end (set after both are placed).
var partner: PipelineLogic = null

## Pipeline length — Manhattan distance between input and output cells.
## Total travel distance in cells = tunnel_length + 1 (includes both endpoint cells).
var tunnel_length: int = 1

## Item buffer — items traveling through the pipeline (only stored on the INPUT end).
...
func configure(def: BuildingDef, p_grid_pos: Vector2i, rotation: int) -> void:
	...
	direction = rotation
	is_input = (def.category == "pipeline")
	set_physics_process(is_input)
	update_sprites()
...
func setup_pair(p_partner: PipelineLogic, p_length: int) -> void:
	partner = p_partner
...
func deserialize_state(state: Dictionary) -> void:
	...
	for item_data in state["tunnel_buffer"]:
		...
		if not GameManager.is_valid_item_id(iid):
			GameLogger.warn("Pipeline at %s: skipped invalid item '%s'" % [grid_pos, iid])
			continue
		var item: Dictionary = {
			id = iid,
