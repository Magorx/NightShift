...
func _build_test_factory() -> void:
	...
	GameManager.place_building(&"source", Vector2i(5, 20), 0)
	# Conveyor chain into smelter input
	GameManager.place_building(&"conveyor", Vector2i(6, 20), 0)
	GameManager.place_building(&"conveyor", Vector2i(7, 20), 0)
	# Smelter anchor at (8, 20): input cell at anchor, output gap at (9,20)
	GameManager.place_building(&"smelter", Vector2i(8, 20), 0)
	# Conveyor in the output gap + chain to sink
	GameManager.place_building(&"conveyor", Vector2i(9, 20), 0)
	GameManager.place_building(&"conveyor", Vector2i(10, 20), 0)
	GameManager.place_building(&"conveyor", Vector2i(11, 20), 0)
	# Sink at the end
	GameManager.place_building(&"sink", Vector2i(12, 20), 0)
