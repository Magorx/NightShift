...
def build_drill():
    ...
    base = add(generate_box(w=1.2, d=1.2, h=0.075, hex_color=STEEL_DK))
    base.name = "BasePlatform"
    apply_texture(base, "metal_plate_02", resolution="1k")

    # Corner feet — small cylinders at each corner
    for i, (fx, fy) in enumerate([(-0.5, -0.5), (0.5, -0.5), (0.5, 0.5), (-0.5, 0.5)]):
    ...
    # ── MAIN HOUSING ──────────────────────────────────────────────────
    body = add(generate_box(w=1.0, d=1.0, h=0.4, hex_color=BODY_MAIN, seam_count=2))
    body.name = "Body"
    body.location = (0, 0, 0.075)
    apply_texture(body, "painted_metal_shutter", resolution="1k")

    # Metal reinforcement band
    band = add(generate_box(w=1.05, d=1.05, h=0.075, hex_color=STEEL_DK))
    band.name = "Band"
    band.location = (0, 0, 0.225)
    apply_texture(band, "metal_plate", resolution="1k")

    # Roof plate
    roof = add(generate_box(w=1.1, d=1.1, h=0.075, hex_color=BODY_ROOF))
    roof.name = "Roof"
    ...
    bolt_positions = [
        # Roof corners
        (0.45, 0.45, 0.55), (-0.45, 0.45, 0.55), (0.45, -0.45, 0.55), (-0.45, -0.45, 0.55),
        # Roof mid-edges
        (0.0, 0.475, 0.55), (0.0, -0.475, 0.55), (0.475, 0.0, 0.55), (-0.475, 0.0, 0.55),
        # Band bolts
        (0.525, 0.35, 0.3), (0.525, -0.35, 0.3), (-0.525, 0.35, 0.3), (-0.525, -0.35, 0.3),
        (0.35, 0.525, 0.3), (-0.35, 0.525, 0.3), (0.35, -0.525, 0.3), (-0.35, -0.525, 0.3),
    ]
    for i, (bx, by, bz) in enumerate(bolt_positions):
        b = add(generate_bolt(head_radius=0.025, head_height=0.015, hex_color=C["rivet"]))
        b.name = f"Bolt_{i}"
        b.location = (bx, by, bz)

    # ── GEARS ─────────────────────────────────────────────────────────
    main_gear = add(generate_cog(outer_radius=0.45, inner_radius=0.25,
                                 teeth=8, thickness=0.15, hex_color=STEEL_LT))
    main_gear.name = "MainGear"
    main_gear.location = (0.55, 0.1, 0.275)
    apply_texture(main_gear, "metal_plate", resolution="1k")

    small_gear = add(generate_cog(outer_radius=0.25, inner_radius=0.175,
                                  teeth=6, thickness=0.15, hex_color=STEEL))
    small_gear.name = "SmallGear"
    small_gear.location = (0.55, -0.4, 0.275)
    apply_texture(small_gear, "metal_plate", resolution="1k")

    # Gear axle caps — small cylinders on gear centers
    for name, pos in [("MainAxle", (0.55, 0.1, 0.36)), ("SmallAxle", (0.55, -0.4, 0.36))]:
        axle = add(generate_cylinder(radius=0.04, height=0.03, segments=8,
                                     hex_color=STEEL_DK))
        axle.name = name
        axle.location = pos

    # ── DERRICK COLUMN ────────────────────────────────────────────────
    derrick = add(generate_cylinder(radius=0.175, height=1.2, segments=12,
                                    hex_color=STEEL))
    derrick.name = "Derrick"
    derrick.location = (0, 0, 0.55)
    apply_texture(derrick, "metal_plate", resolution="1k")

    # Reinforcement rings on derrick at intervals
    for ri, rz in enumerate([0.3, 0.65, 1.0]):
        ring = add(generate_cylinder(radius=0.20, height=0.03, segments=12,
                                     hex_color=STEEL_DK))
    ...
    for si, (sx, sy) in enumerate([(0.5, 0.5), (-0.5, 0.5), (0.5, -0.5), (-0.5, -0.5)]):
        strut = add(generate_box(w=0.04, d=0.04, h=0.5, hex_color=STEEL_DK))
        strut.name = f"Strut_{si}"
        # Angle the strut inward toward the derrick
        strut.location = (sx * 0.325, sy * 0.325, 0.55)
    ...
    cable_runs = [
        ...
        {"start": (-0.25, -0.15, 0.55), "end": (-0.6, -0.3, 0.7), "name": "Cable_0"},
        # Along derrick
        {"start": (0.1, 0.1, 0.65), "end": (0.1, 0.1, 1.6), "name": "Cable_1"},
        # From exhaust to body
        {"start": (-0.2, 0.35, 0.5), "end": (-0.05, 0.15, 0.55), "name": "Cable_2"},
    ]
    for ci, run in enumerate(cable_runs):
        ...
        cable = add(generate_cylinder(radius=0.01, height=length, segments=6,
                                      hex_color=CABLE))
        cable.name = run["name"]
    ...
    # Panel box
    panel = add(generate_box(w=0.25, d=0.04, h=0.175, hex_color=BODY_LIGHT))
    panel.name = "ControlPanel"
    panel.location = (0.15, -0.525, 0.225)
    apply_texture(panel, "painted_metal_shutter", resolution="1k")

    # Gauge — flat cylinder on panel face
    gauge = add(generate_cylinder(radius=0.05, height=0.015, segments=10,
                                  hex_color=GAUGE_FACE))
    gauge.name = "Gauge"
    gauge.rotation_euler = (math.radians(90), 0, 0)
    gauge.location = (0.1, -0.55, 0.275)

    # Gauge rim
    gauge_rim = add(generate_cylinder(radius=0.06, height=0.01, segments=10,
                                      hex_color=COPPER))
    gauge_rim.name = "GaugeRim"
    gauge_rim.rotation_euler = (math.radians(90), 0, 0)
    gauge_rim.location = (0.1, -0.555, 0.275)

    # Toggle switches / knobs on panel
    for ki, kx in enumerate([0.175, 0.225]):
        knob = add(generate_cylinder(radius=0.015, height=0.02, segments=6,
                                     hex_color=RED_WARN if ki == 0 else YELLOW))
        knob.name = f"Knob_{ki}"
        knob.rotation_euler = (math.radians(90), 0, 0)
        knob.location = (kx, -0.55, 0.235)

    # ── VALVE WHEEL (on back pipe) ────────────────────────────────────
    valve = add(generate_cog(outer_radius=0.075, inner_radius=0.05,
                             teeth=5, thickness=0.02, hex_color=RED_WARN))
    valve.name = "ValveWheel"
    valve.rotation_euler = (math.radians(90), 0, 0)
    valve.location = (0.3, 0.49, 0.25)

    # Valve stem
    valve_stem = add(generate_cylinder(radius=0.0125, height=0.05, segments=6,
                                       hex_color=STEEL_DK))
    valve_stem.name = "ValveStem"
    valve_stem.rotation_euler = (math.radians(90), 0, 0)
    valve_stem.location = (0.3, 0.5, 0.25)

    # ── EXHAUST STACK ─────────────────────────────────────────────────
    exhaust = add(generate_cylinder(radius=0.125, height=0.45, segments=10,
                                    hex_color=C["pipe"]))
    exhaust.name = "ExhaustStack"
    exhaust.location = (-0.2, 0.375, 0.475)
    apply_texture(exhaust, "corrugated_iron", resolution="1k")

    # Exhaust cap — wider disc
    exhaust_cap = add(generate_cylinder(radius=0.15, height=0.04, segments=10,
                                        hex_color=STEEL_DK))
    exhaust_cap.name = "ExhaustCap"
    exhaust_cap.location = (-0.2, 0.375, 0.925)
    apply_texture(exhaust_cap, "rusty_metal_02", resolution="1k")

    # Exhaust rain cap — cone above cap with gap
    rain_cap = add(generate_cone(radius_bottom=0.14, radius_top=0.025, height=0.075,
                                 segments=8, hex_color=RUST))
    rain_cap.name = "RainCap"
    rain_cap.location = (-0.2, 0.375, 0.99)
    apply_texture(rain_cap, "rusty_metal_02", resolution="1k")

    # Exhaust mounting bracket — wedge at base
    bracket = add(generate_wedge(w=0.15, d=0.15, h_front=0.0, h_back=0.125,
                                 hex_color=STEEL_DK))
    bracket.name = "ExhaustBracket"
    bracket.location = (-0.2, 0.275, 0.475)
    apply_texture(bracket, "metal_plate", resolution="1k")

    # ── SIDE TANK / CANISTER ──────────────────────────────────────────
    tank = add(generate_cylinder(radius=0.09, height=0.25, segments=10,
                                 hex_color=BODY_LIGHT))
    tank.name = "SideTank"
    tank.rotation_euler = (0, 0, 0)
    tank.location = (0.425, -0.325, 0.075)
    apply_texture(tank, "painted_metal_shutter", resolution="1k")

    # Tank cap
    tank_cap = add(generate_hemisphere(radius=0.09, rings=3, segments=10,
                                       hex_color=BODY_ROOF))
    tank_cap.name = "TankCap"
    tank_cap.location = (0.425, -0.325, 0.325)

    # Tank bands
    for ti, tz in enumerate([0.125, 0.225]):
        tband = add(generate_cylinder(radius=0.10, height=0.015, segments=10,
                                      hex_color=STEEL_DK))
        tband.name = f"TankBand_{ti}"
        tband.location = (0.425, -0.325, tz)

    # Tank feed pipe — connects to body
    tank_pipe = add(generate_pipe(length=0.125, radius=0.02, wall_thickness=0.005,
                                  hex_color=COPPER))
    tank_pipe.name = "TankPipe"
    tank_pipe.rotation_euler = (0, math.radians(90), math.radians(30))
    tank_pipe.location = (0.325, -0.325, 0.2)

    # ── WARNING DETAILS ───────────────────────────────────────────────
    # Hazard stripe band near base
    hazard = add(generate_box(w=1.025, d=1.025, h=0.02, hex_color=YELLOW))
    ...
    side_bolt_positions = [
        # Front face
        (-0.3, -0.505, 0.175), (0.3, -0.505, 0.175),
        (-0.3, -0.505, 0.375), (0.3, -0.505, 0.375),
        # Left face
        (-0.505, -0.3, 0.175), (-0.505, 0.3, 0.175),
        (-0.505, -0.3, 0.375), (-0.505, 0.3, 0.375),
    ]
    for i, (bx, by, bz) in enumerate(side_bolt_positions):
        sb = add(generate_bolt(head_radius=0.02, head_height=0.0125, hex_color=C["rivet"]))
...
def bake_animations(objects):
    ...
    animate_rotation(mg, "idle", duration=2.0, axis='Z',
                     angle_fn=lambda t: 0.015 * math.sin(t * math.pi * 2))
    animate_rotation(sg, "idle", duration=2.0, axis='Z',
                     angle_fn=lambda t: -0.015 * GEAR_RATIO * math.sin(t * math.pi * 2))
    ...
    animate_shake(body, "winddown", duration=1.0, amplitude=0.0075, frequency=8, decay=1.0)
    animate_shake(band, "winddown", duration=1.0, amplitude=0.0075, frequency=8, decay=1.0)
